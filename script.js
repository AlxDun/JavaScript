...
//usuarios.forEach((item)=> {if(item.ciudad=="Bogotá"){console.log(item.nombre)}})

// Obtener un arreglo con los correos electrónicos.
//let correos = []
//usuarios.forEach((item) => {correos.push(item.correo)}); console.log(correos)

// Sumar las edades de todos los usuarios.
//let sumatoria = 0
//usuarios.forEach((item) => {sumatoria+=item.edad}); console.log(`La sumatoria de las edades es: ${sumatoria}`)

// Encontrar el usuario con mayor edad.
// let edadMayor = 0
// let mayor
// usuarios.forEach((item) => {if(item.edad>edadMayor){edadMayor=item.edad; mayor=item}}); console.log(`La persona mas mayor es ${mayor.nombre}, con ${edadMayor} años.`)
